...
class AttentionLSEncoder(nn.Module):
    ...
    def forward(self, X, event_emb, g, mask):
        ...
        Q = self.split_heads(self.W_q(X)).mul(1.0 / math.sqrt(self.d_k))
        ...
        if self.num_landmarks > 0:
            ...
            head_scores = head_scores.view(bsz, seqlen, self.num_head, self.num_landmarks).permute(
                0, 2, 3, 1
            )
        ...
        if self.dual_ln_s is not None and K_compress is not None:
            K_compress = self.dual_ln_s(
                K_compress.transpose(1, 2).contiguous().view(bsz, -1, self.num_head * d_model)
            )
            K_compress = self.split_heads(K_compress)
            V_compress = self.dual_ln_s(
                V_compress.transpose(1, 2).contiguous().view(bsz, -1, self.num_head * d_model)
            )
        ...
        if attn_compress is None:
            ...
            attn_compress = torch.zeros(attn_compress.size())
            # print(attn_compress.size())
        ...
        if win_attn_weights is not None:
            ...
            if self.window_size > 0:
                win_attn_probs = win_attn_probs.view(
                    bsz, self.num_head, seqlen // self.window_size, self.window_size, -1
                )
