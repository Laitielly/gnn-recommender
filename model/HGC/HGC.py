...
class HGCEncoder(nn.Module):
    def __init__(self, d_model, n_head):
        ...
        self.heads = nn.ModuleList([HGCLayer(d_model, d_model) for _ in range(n_head)])
    ...
    def get_latest_k_mask(self, seq, k):
        ...
        bz, seq_len = seq.size()
        mask = torch.zeros(seq_len, seq_len, device="cuda:0")  # device='cuda:0'
...
class HGCLayer(nn.Module):
    ...
    def forward(self, output, local_cor, cor_matrix, event_type):
        ...
        outputs = torch.matmul(
            correlation_output.transpose(1, 2), cor_matrix[event_type - 1]
        ).mean(2)
