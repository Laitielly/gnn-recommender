...
class Encoder(nn.Module):
    def __init__(self, num_types, d_model, n_layers, n_head, dropout):
        ...
        self.hgc_layers = nn.ModuleList([HGCEncoder(d_model, n_head) for _ in range(n_layers)])
        ...
        if C.ENCODER == "Transformer":
            ...
            self.layer_stack = nn.ModuleList(
                [THPEncoder(d_model, n_head) for _ in range(n_layers)]
            )
        elif C.ENCODER == "NHP":
...
class Predictor(nn.Module):
    ...
    def forward(self, user_seq_rep, embeddings, user_gra_rep, user_gra2_rep):
        ...
        if (
            C.DATASET == "Beauty"
            or C.DATASET == "Yelp2023"
            or C.DATASET == "Food.com"
            or C.DATASET == "Amazon-book"
            or C.DATASET == "Kuairand-1k"
            or C.DATASET == "Zvuk"
        ):
...
class Model(nn.Module):
    ...
    def forward(self, user_id, event_type, matrices):
        ...
        attn_output, user_seq_rep, user_gra_rep, user_gra2_rep = self.encoder(
            event_type, enc_output, matrices
        )
