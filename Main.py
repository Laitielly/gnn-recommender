...
def train_epoch(model, user_dl, matrices, optimizer, opt):
    ...
    for batch in tqdm(user_dl, mininterval=2, desc="  - (Training)   ", leave=False):
        ...
        """ prepare data """
        user_idx, event_type, event_times, sentiment, test_label = map(
        ...
        if C.ENCODER in {"THP", "NHP"}:
            event_ll, non_event_ll = Utils.log_likelihood(
                model, att_output, event_times, sentiment
            )
            event_loss = -torch.mean(event_ll - non_event_ll)
            if C.DATASET in C.DICT:
                loss += event_loss / C.DICT[C.DATASET]
...
def train(model, data, optimizer, scheduler, opt):
    ...
    print(
        "\rP@k:{pre},    R@k:{rec}, \n"
        "(Best)map@k:{map_}, ndcg@k:{ndcg}".format(
            pre=best_[0], rec=best_[1], map_=best_[2], ndcg=best_[3]
        )
    )
    print("-" * 40, "BEST", "-" * 40, "\n")
...
def main(trial):
    ...
    if C.DATASET == "ml-1M":
        opt.epoch, opt.batch_size = 30, 32
    elif C.DATASET == "Yelp2023":
        opt.epoch, opt.batch_size = 15, 16
    elif C.DATASET == "Food.com":
        opt.epoch, opt.batch_size = 6, 32
    elif C.DATASET == "Beauty":
        opt.epoch, opt.batch_size = 16, 32
    elif C.DATASET == "Amazon-book":
        opt.epoch, opt.batch_size = 16, 16
