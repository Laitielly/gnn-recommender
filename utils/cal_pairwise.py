...
def read_interaction():
    ...
    train_data = open(
        directory_path + "{dataset}_train.txt".format(dataset=C.DATASET), "r"
    ).readlines()
    train_data.extend(
        open(directory_path + "{dataset}_tune.txt".format(dataset=C.DATASET), "r").readlines()
    )

    interaction_matrix = torch.zeros((C.USER_NUMBER, C.ITEM_NUMBER), device="cuda:0")  #
    adjacency_matrix = torch.zeros(
        (C.ITEM_NUMBER, C.ITEM_NUMBER), device="cuda:0"
    )  # device='cuda:0'
