...
def print_test_result():
    ...
    for i, k in enumerate(parse.topks):
        print(
            f"ndcg@{k:d} = {test_ndcg[i]:f}, recall@{k:d} = {test_recall[i]:f}, pre@{k:d} = {test_pre[i]:f}."
        )
...
def valid(epoch):
    ...
    for i, k in enumerate(parse.topks):
        print(
            f"[{epoch:d}/{args.epochs:d}] Valid Result: ndcg@{k:d} = {valid_ndcg[i]:f}, recall@{k:d} = {valid_recall[i]:f}, pre@{k:d} = {valid_pre[i]:f}."
        )
...
best_valid_ndcg, best_epoch = 0.0, 0
test_pre, test_recall, test_ndcg = (
    torch.zeros(len(args.topks)),
    torch.zeros(len(args.topks)),
    torch.zeros(len(args.topks)),
)
valid(epoch=0)
for epoch in range(1, args.epochs + 1):
    ...
    if epoch % args.valid_interval == 0:
        if not valid(epoch) and epoch - best_epoch >= args.stopping_step * args.valid_interval:
            torch.save(model.state_dict(), os.path.join(args.save_dir, args.data, "model.pth"))
            break
print("---------------------------")
print_test_result()
