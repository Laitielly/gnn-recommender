...
class MyDataset(Dataset):
    def __init__(self, train_file, valid_file, test_file, device):
        ...
        print(
            "test: %d pos + %d neg." % (self.test_pos_user.shape[0], self.test_neg_user.shape[0])
        )
        #
            ...
            self._A_neg = torch.sparse_coo_tensor(
                torch.cat(
                    [
                        ...
                        torch.stack([self.train_neg_item + self.num_users, self.train_neg_user]),
                    ],
                    dim=1,
                ),
    ...
    def sample(self):
        if self._indices is None:
            ...
            d = 1.0 / d
            self._values = (
                torch.ones(self._indices.shape[1]).to(parse.device)
                * d[self._indices[0]]
                * d[self._indices[1]]
            )
        res_X, res_Y = [], []
        ...
        for hop in range(args.sample_hop):
            ...
            next_indices = (
                ...
                - (torch.rand(X.shape[1]).to(parse.device) * self._counts[X[1]]).long()
                - 1
            )
