...
device = torch.device(
    f"cuda:{args.device:d}" if args.device != -1 and torch.cuda.is_available() else "cpu"
)
...
print("Data Setting")
train_file = os.path.join(args.data_dir, args.data, "train.txt")
valid_file = os.path.join(args.data_dir, args.data, "valid.txt")
test_file = os.path.join(args.data_dir, args.data, "test.txt")
