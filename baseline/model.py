...
class Attention(nn.Module):
    def __init__(self):
        ...
        self.sqrt_dim = 1.0 / torch.sqrt(torch.tensor(args.hidden_dim))
        self.sqrt_eig = 1.0 / torch.sqrt(torch.tensor(args.eigs_dim))
        self.my_parameters = [
...
class Model(nn.Module):
    def __init__(self, dataset):
        ...
        self.optimizer = torch.optim.Adam(self.my_parameters, lr=args.learning_rate)
    ...
    def evaluate(self, test_pos_unique_users, test_pos_list, test_neg_list):
        ...
        with torch.no_grad():
            ...
            for i in range(0, users.shape[0], args.test_batch_size):
                ...
                pre, recall, ndcg = utils.test(rating, test_pos_list[i : i + args.test_batch_size])
                all_pre += pre
                all_recall += recall
                all_ndcg += ndcg
    ...
    def loss_one_batch(self, pos_u, pos_i, pos_j, neg_u, neg_i, neg_j):
        ...
        if args.beta == 0:
            ...
            scores = torch.concat(
                [pos_scores_uj - pos_scores_ui, args.beta * (neg_scores_uj - neg_scores_ui)], dim=0
            )
